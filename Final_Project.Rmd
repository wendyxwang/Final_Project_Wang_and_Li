...
**Overview of data**
* We delte any genes with more than 50% missing values.
* Now total 2938 antibodies(genes), there may exist duplication. 
```{r,echo=FALSE,reading data}
#load packages
library(limma)
library(qvalue)
...
```{r,echo=FALSE,batch}
batch.m<-ma$M
case +Hybrid.day+box+position+gender
image(batch.m[,info$gender==1])
image(batch.m[,info$gender==0])

Pvalue<-NULL;
for(i in 1:dim(batch.m)[1]){
  fit<-lm(batch.m[i,]~as.factor(info$Hybrid.day))
  Pvalue<-c(Pvalue,anova(fit)$"Pr(>F)"[1])
}
...
```{r, fig.height=8,fig.width=15,  batch effects}
