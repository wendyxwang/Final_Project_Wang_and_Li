...
library("knitr")
opts_chunk$set(tidy=TRUE, size='Huge',tidy.opts=list(blank=FALSE, width.cutoff=80), cache=TRUE, messages=FALSE)
```
...
```{r batch,echo=FALSE,fig.width=15,fig.height=8,cache=TRUE}
batch.m<-ma$M
...
- The therethold value for q value and bonferroni test  is 0.001.  
...
```{r, fig.height=8,fig.width=15,cache=TRUE,echo=FALSE}
...
```{r Qvalueplot,echo=FALSE,cache=TRUE,fig.height=15,fig.width=15,fig.align='center'}
...
- Moderated T test using R Limma package
*Plots*
```{r, echo=FALSE,cache=TRUE,fig.align='center',fig.height=8,fig.width=15}
p.cut <- 0.01
q.cut<-0.001
